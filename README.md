...
In the currently published 1.0.0, `--help` and `--version` do not require a key and write no error output. Results pagination follows canonical `cursor`/`nextCursor` responses and remains compatible with `offset` responses; network, 429, and server retries are bounded. The unpublished 1.0.1 candidate additionally makes invalid/truncated successful responses and repeated cursors fail explicitly instead of returning partial results as complete.
...
BOUNCELESS_API_KEY=blc_example npx bounceless-mcp
```
