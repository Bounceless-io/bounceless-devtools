...
export function createProgram(client: BouncelessClient, io: Io = defaultIo): Command {
  const program = new Command().name('bounceless').description('Bounceless email verification CLI').version('1.0.2').exitOverride().configureOutput({ writeOut: s => io.stdout(s.trimEnd()), writeErr: s => io.stderr(s.trimEnd()) });
